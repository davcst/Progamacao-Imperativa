...
void RemoverSolicitacao() {
    char P[7];
    printf("Placa da moto para remover a solicitacao: ");
    scanf(" %[^\n]s", P);
    int Pos = -1; 
    ...
    if(Pos == -1)
        ...
        if(VZonda[Pos].Status == '0') {
            ...
            printf("Nao e possivel remover a solicitacao. O servico ja foi iniciado.\n");
    }
}
...
void ConcluirServico(){
    char P[7];
    printf("Placa da moto para concluir o servico: ");
    scanf(" %[^\n]s", P);
    int Pos = -1; 
    for(int i = 0; i <= Quant; i++){
        if(strcmp(VZonda[i].Placa, P) == 0)
    ...
    if(Pos == -1)
        ...
        if(VZonda[Pos].Status == '1'){
            printf("Qual o preco final do servico? ");
            scanf("%f", &VZonda[Pos].Preco);
            VZonda[Pos].Status = '3';
            printf("Servico concluido com sucesso!\n");
        }
        else
            printf("Nao e possivel concluir o servico. O servico nao foi iniciado.\n");
    }
}
    ...
    if(Quant == -1){
        ...
        for(int i = 0; i <= Quant; i++){
            ...
            if(VZonda[i].Status == '3'){
                ...
                printf("\n Preco: Servico nao concluido");
            }
            printf("\n --------------------- \n\n");
        }
        printf("\nValor total obtido com os servicos: %.2f\n", total);
    }
}


void ConsultarServConcluidos(){
    printf("\n\n >>> Motos Zonda <<< \n\n");
    int encontrado = 0;
    ...
    if(encontrado == 0)
        printf("Nao ha serviços concluidos registrados.\n");
}

void BuscarServPorPlaca(){
    char P[7];
    printf("Digite a placa da moto para buscar o servico: ");
    scanf(" %[^\n]s", P);
    int encontrado = 0;
    printf("\n\n >>> Motos Zonda <<< \n\n");
    ...
    if(encontrado == 0)
        printf("Nao foram encontrados servicos para a placa especificada.\n");
}
...
int main(){
  ...
  do{
    ...
    case 7: ConsultarServConcluidos(); break;
    case 8: BuscarServPorPlaca(); break;
	case 9: break;}}	  	
	while (Opcao!=9);
  return 0;}